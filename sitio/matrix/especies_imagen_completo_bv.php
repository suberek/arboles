...
$especies_imagen_completo_blobview = NULL; // Initialize page object first

class cespecies_imagen_completo_blobview extends cespecies {
	...
	// Page object name
	var $PageObjName = 'especies_imagen_completo_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'especies');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('especies', 'x_imagen_completo');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('imagen_completo');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('imagen_completo')));
				if (trim(strval($this->Recordset->fields('imagen_completo'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('imagen_completo') . "\"");
				}
...
// Create page object
if (!isset($especies_imagen_completo_blobview)) $especies_imagen_completo_blobview = new cespecies_imagen_completo_blobview();

// Page init
$especies_imagen_completo_blobview->Page_Init();

// Page main
$especies_imagen_completo_blobview->Page_Main();
?>
<?php
$especies_imagen_completo_blobview->Page_Terminate();
?>
