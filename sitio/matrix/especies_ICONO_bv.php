...
$especies_ICONO_blobview = NULL; // Initialize page object first

class cespecies_ICONO_blobview extends cespecies {
	...
	// Page object name
	var $PageObjName = 'especies_ICONO_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('especies', 'x_ICONO');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('ICONO');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('ICONO')));
				if (trim(strval($this->Recordset->fields('ICONO'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('ICONO') . "\"");
				}
...
// Create page object
if (!isset($especies_ICONO_blobview)) $especies_ICONO_blobview = new cespecies_ICONO_blobview();

// Page init
$especies_ICONO_blobview->Page_Init();

// Page main
$especies_ICONO_blobview->Page_Main();
?>
<?php
$especies_ICONO_blobview->Page_Terminate();
?>
